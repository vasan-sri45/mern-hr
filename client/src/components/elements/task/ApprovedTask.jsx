...
const Loader = React.memo(() => (
  <div className="flex justify-center items-center py-10"><p className="text-lg text-gray-700">Loading approved tasks...</p></div>
));
Loader.displayName = 'Loader';

const ErrorDisplay = React.memo(({ error }) => (
  <div className="flex justify-center items-center py-10"><p className="text-lg text-red-600">Failed to load tasks: {error}</p></div>
));
...
const ApprovedTask = () => {
  ...
  const approvedTasks = useMemo(() => {
    ...
    let filtered = tasks.filter(t => t.checkList === 'approved');
  ...
  const { completed, notCompleted } = useMemo(() => computeStatusCounts(approvedTasks), [approvedTasks]);
  ...
  if (!loading && approvedTasks.length === 0) {
    return (
      <div className="text-center py-10">
        <h2 className="text-xl font-semibold">No Approved Tasks</h2>
        <p className="text-gray-500 mt-2">There are currently no tasks marked as approved for you.</p>
      </div>
    );
  }
